...
export const stationChainDiscountsColumns = (pageNumber, pageSize, deleteStationChainDiscount, deleteLoading, openModal) => [
    {
        title: `#`,
        key: 'numberOfRow',
        fixed: 'left',
        ...
        render: (row) => {
            return (
              <Flex gap="small" justify="center" align="center" wrap>
                  {row?.stationChains?.map((chain, index) => {
                      return <Tag className='m-0' key={index}>{chain.name}</Tag>;
                  })}
              </Flex>
            );
        }
            ...
            return (
                <div className='flex justify-center'>
                    <Popconfirm
                        ...
                        onConfirm={() => deleteStationChainDiscount(row)}
                        okText="Yes"
                        cancelText="No"
