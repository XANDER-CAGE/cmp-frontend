...
import DownloadExcelReportV2Dropdown from '../../components/invoicing/DownloadExcelReportV2Dropdown';
import React from 'react';

export const invoicingColumns = (pageNumber, pageSize, deleteInvoice, deleteLoading, downloadInvoice, openInvoiceDetailsModal, openEditInvoiceModal, permissions, regenerateInvoiceReportByVersion, openCompanyDetailsModal,
    showDeleteOperation = true, showEditOperation = true, showDetailOperation = true, showPdfOperation = true, showExcelOperation = true) => [
        {
            title: `#`,
            key: 'numberOfRow',
            fixed: 'left',
            ...
            render: (row) => {
                return (
                    <div className='flex justify-center'>
                        {
                          Authorize(permissions, [PERMISSIONS.INVOICE.CANCEL]) && showDeleteOperation ? (
                                <Popconfirm
                        ...
                        {
                            Authorize(permissions, [PERMISSIONS.INVOICE.EDIT]) && showEditOperation ? (
                                <div className='icon' onClick={() => openEditInvoiceModal(row)}>
                                    <MdEdit />
                                </div>
                            ) : null
                        }
                        ...
                        {
                          Authorize(permissions, [PERMISSIONS.INVOICE.EDIT]) && showEditOperation ? (
                            <Tooltip title={'Change Invoice Design'}>
                              <Popconfirm
                                placement="topRight"
                                title="Change Invoice Design"
                                description={`Are you sure to change invoice design to ${row?.reportDesignVersion === 'V1' ? 'V2 - New Design' : 'V1 - Old Design' }?`}
                                onConfirm={() => regenerateInvoiceReportByVersion(row?.id, row?.reportDesignVersion === 'V1' ? 'V2' : 'V1')}
                              >
                                <div className="icon">
                                  <LuRefreshCcw />
                                </div>
                        ...
                        {
                          Authorize(permissions, [PERMISSIONS.INVOICE.EXPORT]) && showPdfOperation ? (
                                <Tooltip title={'PDF'}>
                                    <div className='icon' onClick={() => downloadInvoice(row, 'PDF')}>
                                        <FaFilePdf />
                                    </div>
                                </Tooltip>
                            ) : null
                        ...
                        {
                            Authorize(permissions, [PERMISSIONS.INVOICE.EXPORT]) && showExcelOperation && row?.reportDesignVersion === 'V1' ? (
                                <Tooltip title={'Excel'}>
                                    <div className='icon' onClick={() => downloadInvoice(row, 'EXCEL')}>
                                        <FaFileExcel />
                                    </div>
                                </Tooltip>
                            ) : null
                        ...
                        {
                          Authorize(permissions, [PERMISSIONS.INVOICE.EXPORT]) && showExcelOperation && row?.reportDesignVersion === 'V2' ? (
                            <DownloadExcelReportV2Dropdown invoice={row} downloadInvoice={downloadInvoice} />
                          ) : null
                        }
                    </div>
                )
