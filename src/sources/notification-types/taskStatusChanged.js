...
import { setTagColor, setTashkentTime } from '../../utils'

const TaskStatusChanged = (props) => {
    ...
    return (
        <Card
            ...
            title={
                <>
                    <Link href={`/tasks?view=true&task_id=${item?.TaskId}`} className='text-[#00000060] text-[12px]'>Tasks &gt; {item?.TaskTitle}</Link>
        ...
        >
            <div className='flex items-center justify-between'>
                <div className='flex items-center'>
                    ...
                    <span className='ml-2'><b>{item?.ChangedByUsername}</b> changed Status to:</span>
                    <Tag
                        color={setTagColor(item?.NewStatus)}
                        className='ml-3'
                    >
                        {item?.NewStatus}
                    </Tag>
...
export default TaskStatusChanged