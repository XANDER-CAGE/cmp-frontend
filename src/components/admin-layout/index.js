...
const AdminLayout = () => {
  ...
  // Эффект для установки цвета фона при изменении темы
  useEffect(() => {
    const layoutContainer = document.querySelector('.admin-layout-container');
    if (layoutContainer) {
      layoutContainer.style.backgroundColor = isDarkMode ? '#1f2937' : '#f3f4f6';
    }
  }, [isDarkMode]);
    ...
    <div className="admin-layout-container">
      ...
      <div className="admin-layout-main">
        <Sidebar
          collapsed={collapsed}
        />
        <div className={`content-area ${collapsed ? 'content-expanded' : ''}`}>
          <Routes>
            {/* PROFILE SETTINGS*/}
