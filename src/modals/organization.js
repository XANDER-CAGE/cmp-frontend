...
const OrganizationsModal = (props) => {
    const { isOpenModal, getOrganizations, closeModal, editId } = props
    ...
    const getOrganizationById = async () => {
        try {
            const response = await http.get(`Organizations/${editId}`)
    ...
    const submitForm = async (values) => {
        ...
        try {
            const response = editId ? await http.put(`Organizations/${editId}/`, { ...values, id: editId }) : await http.post('Organizations', values)
            if (response?.success) {
    ...
    return (
        <Modal
            ...
            centered
            width={1000}
            title={`${editId ? "Edit" : "Add"} Organization`}
            footer={[]}
            closeIcon={null}
        >
            <Form
