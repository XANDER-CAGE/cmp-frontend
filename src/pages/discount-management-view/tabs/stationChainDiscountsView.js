...
const StationChainDiscountsView = () => {
    ...
    const filters = useMemo(
        () => ({
            ...
            stationChainIds: selectedStationChains,
            searchTerm,
            pagination: {
                pageSize,
                pageNumber
        ...
        [dateStrings, searchTerm, selectedStationChains, pageSize, pageNumber],
    )
    ...
    const getStationDiscountsView = async () => {
        setIsLoading(true)
        try {
            const response = await http.post('StationChainDiscounts/filter', filters)
            setStationChainDiscounts(response?.data)
    ...
    const days = useMemo(
        () =>
        (groupByKey(
            stationChainDiscounts?.days?.map((date, index) => {
                return {
                    index,
                    date,
                    day: date.slice(8),
        ...
        [stationChainDiscounts],
    )

    const items = useMemo(
        () =>
            stationChainDiscounts?.items?.map((item) => ({
                ...item,
                key: item?.station?.id,
            })),
        [stationChainDiscounts],
...
export default StationChainDiscountsView