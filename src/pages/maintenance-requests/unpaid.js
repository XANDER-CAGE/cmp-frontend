...
const Unpaid = () => {
    ...
    const filters = useMemo(() => {
          return {
              ...
              status,
              sorts,
              paymentStatus: paymentStatus ? paymentStatus : 11,
              hasManualInvoice: manualInvoice
          }
      },
      [pageNumber, pageSize, dateStrings, organizationId, efsAccountId, companyId, searchTerm, status, paymentStatus, sorts, manualInvoice]
    )
    ...
    return (
      <div>
          ...
          <CustomTable
            name="maintenances"
            columns={maintenancesColumns(pageNumber, pageSize, deleteMaintenanceRequest, deleteLoading, openModal, openModalAttachment, true, permissions, true, downloadInvoice, openCompanyDetailsModal )}
            data={maintenances}
            size="small"
